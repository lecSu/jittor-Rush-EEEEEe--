...
class Discriminator(nn.Module):
    ...
    def __init__(self):
        ...
        self.model = nn.Sequential(nn.Linear((opt.n_classes + int(np.prod(img_shape))), 512), 
                                   ...
                                   nn.Linear(32,1), 
                                   nn.Sigmoid()
                                #    nn.Conv2d(img_shape[1],16,kernel_size=3),
                                # nn.LeakyReLU(0.2), 
                                # nn.Dropout(0.4), 
...
#         g_loss = 3*adversarial_loss(validity, valid)
#         g_loss.sync()
...
#         d_real_loss = adversarial_loss(validity_real,labels)#"""TODO: 计算真实类别的损失函数"""

#         validity_fake = discriminator(gen_imgs.stop_grad(), gen_labels)
#         d_fake_loss = adversarial_loss(validity_fake,labels)#"""TODO: 计算虚假类别的损失函数"""
...
number ="15056064473" #TODO: 写入你注册时绑定的手机号（字符串类型）
n_row = len(number)
